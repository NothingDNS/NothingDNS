...
- **UDP on secondary / alias host IPs timed out**: with `bind: [0.0.0.0]` the
  server accepted queries on extra `/32` addresses but replied from the primary
  IP, so clients dropped the response (TCP still worked). Linux UDP sockets now
  use `IP_PKTINFO` / `IPV6_RECVPKTINFO` so replies keep the inbound local
...
- **Cluster Topology missing link to the top follower**: healthy Raft edges used
  a horizontal SVG `linearGradient`, which paints nothing on a vertical line
  (zero-width object bounding box). Links now use a solid stroke from circle
  edge to edge so every follower — including the top one — shows a connector.
